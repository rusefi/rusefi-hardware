...
init0:      dfcsct dac3;
            stgn gain19.3 ssoc;                     * Set the gain of the opamp of the current measure block 1 
            ldjr1 eofp0;                            * Load the eoinj line label Code RAM address into the register jr1 
...
idle0:      joslr fp_start start8;                  * Perform an actuation
            jmpf jr1;                               * If more than 1 start active at the same time(or none), no actuation

fp_start:   dfsct hs7 ls8 undef;                    * Set the 2 shortcuts: VBAT, LS
            jmpr peak0;                             * Jump to launch phase

* ### Launch peak phase on bat ###
peak0:      load HPFP_Ipeak dac_ssoc _ofs;          * Load the boost phase current threshold in the current DAC
            cwer hold0 ocur row2;                   * Jump to hold phase when current is over threshold
...
hold0:      ldcd rst _ofs keep keep HPFP_Thold_tot c1;   * Load the length of the total hold phase in counter 1
            load HPFP_Ihold dac_ssoc _ofs;               * Load the hold current threshold in the DAC
            cwer eofp0 tc1 row2;                    * Jump to eoinj phase when tc1 reaches end of count
...
eofp0:      stos off off keep;                      * Turn VBAT off, LS off
